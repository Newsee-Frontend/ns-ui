<template>
  <div class="menu-item">
    ...
    <collapse-transition>
      ...
      >
        <nav-menu-node
          v-for="(child,index) in node.childNodes" :key="index"

          :node="child" :keyRefer="keyRefer"
  ...
  export default {
    ...
    created() {
      ...
      this.$on('menu-node-click', node => {
        ...
        console.log('当前手风琴模式：对 menu-node-click 事件回抛进行接收，判断 节点 node - branchID字段值，并关闭不对应的节点');
