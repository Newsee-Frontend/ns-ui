  ...
  <li v-if="itemVisible" :class="liClass">
    <div class="tree-node-el" :draggable="draggable" @dragstart="drag(item, $event)">
      ...
      <span @click="expandNode(item)" v-else>
        <span v-if="hasChildren || item.async">
           <icon-svg v-if="item.iconClass" :icon-class="item.iconClass"></icon-svg>
           <icon-class v-else :icon-class="item.expanded ? 'close-block' : 'open-block'"></icon-class>
        </span>
        <span v-else>
           <icon-svg v-if="item.iconClass" :icon-class="item.iconClass"></icon-svg>
